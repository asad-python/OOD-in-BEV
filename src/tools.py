...
class SimpleLoss(torch.nn.Module):
    ...
    def forward(self, ypred, ytgt):
        # ypred should be of shape (N, C, H, W) and ytgt should be of shape (N, H, W)
        loss = self.loss_fn(ypred, ytgt)
        return loss
...
def get_batch_iou(preds, binimgs, num_classes=3):
    ...
    with torch.no_grad():
        ...
        for cls in range(num_classes):
            ...
            if union == 0:
                iou_per_class.append(1.0)  # If no ground truth, consider IoU as 1
            else:
...
def get_val_info(model, valloader, loss_fn, device, masks=None, use_tqdm=False):
    model.eval()
    total_loss = 0.0
    total_iou = 0.0
    print('running eval...')
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            if masks is not None:
                mask_batch = masks[batch_idx * allimgs.size(0):(batch_idx + 1) * allimgs.size(0)]
            else:
                mask_batch = None

            preds = model(allimgs.to(device), rots.to(device),
                          trans.to(device), intrins.to(device), post_rots.to(device),
                          post_trans.to(device), masks=mask_batch)
            binimgs = binimgs.to(device).squeeze(1).long()  # Squeeze the extra dimension and convert to Long
            ...
            total_iou += iou * preds.shape[0]

    model.train()
    return {
            'loss': total_loss / len(valloader.dataset),
            'iou': total_iou / len(valloader.dataset),
            }
